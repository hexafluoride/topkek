...
namespace Exchange.WebSockets
{
    public class ClientConnection : WebSocketConnection
    {
        ...
        public ClientConnection(TcpClient client, bool tls = false, string tls_host = "")
        {
            Client = client;
            ...
            if (tls)
            {
                var ssl = new SslStream(ns, false, (a, b, c, d) => { return true; });
                ssl.AuthenticateAsClient(tls_host);
                NetworkStream = ssl;
            }
            else
                NetworkStream = ns;
        ...
        public void PerformHandshake(string host, string path, string origin)
        {
